...
function_init()
{
    if [ ! -f /usr/local/nginx/conf/vhost/default.conf ]; then
        {
            ...
            echo '            fastcgi_read_timeout 99999;';\
            echo '            fastcgi_send_timeout 99999;';\
    ...
    sed -e "s@^;error_log = php_errors.log.*@error_log = /var/www/logs/internal_php_errors.log@" /usr/local/php/etc/php.ini -i;

    if [ -n "$MEMCACHED_CONFIG" ]; then
        sed \
        -e "s#127.0.0.1#`echo $MEMCACHED_CONFIG|cut -d ':' -f1`#g" \
        -e "s#11211#`echo $MEMCACHED_CONFIG|cut -d ':' -f2`#g" \
        /var/www/html/application/config/memcached.php -i;

        sed \
        -e "s@^session.save_handler.*@session.save_handler = memcached@" \
        -e "s@^;session.save_path.*@session.save_path = \"${MEMCACHED_CONFIG}\"@" \
        -e "s@^session.gc_maxlifetime.*@session.gc_maxlifetime = 43200@" \
        /usr/local/php/etc/php.ini -i;
    fi
}
