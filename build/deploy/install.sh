...
WEB_IMAGE=ccr.ccs.tencentyun.com/yunker/xindai
...
if [ "$FLAG" = "install" ]; then
	...
	WEB_CONTAINER_ID=`docker run -d -P --rm --env-file "${CUR_DIR}/config.env" -v ${CUR_DIR}/logs:/var/www/logs  -v ${CUR_DIR}/upload:/var/www/html/upload ${WEB_IMAGE}:${DEPLOY_VERSION}`
